...
function AgeRatingG() {
  const [animes, setAnimes] = useState<AgeRatingGResponse[]>([]);

  useEffect(() => {
    const fetchGAnime = async () => {
      try {
        const response = await fetch(
          "https://kitsu.io/api/edge/anime?filter[ageRating]=G",
          {
            headers: {
        ...
        const json: AgeRatingGResponse = await response.json();
        setAnimes(json.data);
      } catch (error: any) {
