...
function GenreActionAnime() {
  const [animes, setAnimes] = useState<GenreActionAnimeResponse[]>([]);

  useEffect(() => {
    const fetchGenreAnime = async () => {
      try {
        const response = await fetch(
          "https://kitsu.io/api/edge/anime?filter[categories]=action",
          {
            headers: {
        ...
        const json: GenreActionAnimeResponse = await response.json();
        setAnimes(json.data);
      } catch (error: any) {
...
export default GenreActionAnime;