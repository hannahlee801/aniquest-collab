...
function OVAsComp() {
  const [animes, setAnimes] = useState<OVAsResponse[]>([]);

  useEffect(() => {
    const fetchOVAs = async () => {
      try {
        const response = await fetch(
          "https://kitsu.io/api/edge/anime?filter[subtype]=ova&page[limit]=20",
          {
            headers: {
        ...
        const json: OVAsResponse = await response.json();
        setAnimes(json.data);
      } catch (error: any) {
