...
type GenreRomanceAnimeResponse = {
  id: string;
  attributes: {
    posterImage: {
      small: string;
...
function GenreRomanceAnime() {
  const [animes, setAnimes] = useState<GenreRomanceAnimeResponse[]>([]);
  ...
  useEffect(() => {
    const fetchGenreAnime = async () => {
      try {
        ...
        const json: GenreHorrorRomanceResponse = await response.json();
        setAnimes(json.data);
      } catch (error: any) {
...
export default GenreRomanceAnime;