...
function AgeRatingPG() {
  const [animes, setAnimes] = useState<AgeRatingPGResponse[]>([]);

  useEffect(() => {
    const fetchPGAnime = async () => {
      try {
        const response = await fetch(
          "https://kitsu.io/api/edge/anime?filter[ageRating]=PG",
          {
            headers: {
        ...
        const json: AgeRatingPGResponse = await response.json();
        setAnimes(json.data);
      } catch (error: any) {
